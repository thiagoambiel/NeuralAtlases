...
class Conv2dIBNormRelu(nn.Module, ABC):
...
class LRBranch(nn.Module, ABC):
    """ Low Resolution Branch of MODNet"""

    def __init__(self, backbone: MobileNetV2Backbone):
...
class FusionBranch(nn.Module, ABC):
...
class MODNetArch(nn.Module, ABC):
    """ Architecture of MODNet"""

    def __init__(self, in_channels: int = 3, hr_channels: int = 32):
        super(MODNetArch, self).__init__()
...
class MODNet(nn.Module, ABC):
    def __init__(self, checkpoint: str):
        super(MODNet, self).__init__()

        self.model = MODNetArch()
    ...
    def forward(self, img: np.array):
        img = self.normalize(img).float().to(self.device)

        img_h, img_w = img.shape[1:3]
        img_rh, img_rw = self.scale_boundaries(img_h, img_w, ref_size=512)

        img = F.interpolate(img.unsqueeze(0), size=(img_rh, img_rw), mode='area')

        mask = self.model(img)

        mask = F.interpolate(mask, size=(img_h, img_w), mode='area').squeeze()
